...
FILE **threads_logfiles;

// FILE **create_thread_files(int index){
//     static FILE *thread_files[MAX_THREADS_NUM];
//     char file_name[13];

//     if (index<10){
//         snprintf(file_name, 13, "thread0%d.txt",index);
//     }
//     else {
//         snprintf(file_name, 13, "thread%d.txt", index);
//     }
//     thread_files[index] = fopen(file_name, "w+");
//     if (thread_files[index] == NULL){
//         printf("Failed in creating thread file %d.\n", index);
//         exit(1);
//     }
//     fputs("0\0", thread_files[index]);
//     rewind(thread_files[index]);
//     return thread_files;
// }
...
void *works_func(void *arg){
    ...
    while(1)
    {
        ...
        busy[thread_id] = 1;
        job_node *last_job; //= get_job_from_queue(head); WE NEED TO EXECUTE FROM THE FIRST JOB TO LAST. SO IN THE DISPACHER NEED TO ALWAYS MOVE "HEAD" TO  BE LAST  
        strcpy(line, last_job->text);
        //strcpy(original_line, line);
        free(last_job); //remove last job from the queue so no other thread will take it, and execute it.
        pthread_mutex_unlock(&mutex);

        //parse_and_execute(line, thread_id);
...
void init_threads(int num_of_threads, int log){
    pthread_t tid;
    thread_data td;
    ...
    for (int i=0; i<num_of_threads; i++){
        ...
        if (log == 1){
            //threads_logfiles = create_thread_files(i);
        } //need to create thread file for each thread
    // if (log_handler == 1) need to create dispatcher file
    }
}
